...
namespace llarp
{
  ...
  FindFreeTun()
  {
    ...
    if(num == 255)
    {
      llarp::LogError("Could not find any free coiniclestun interface names");
      return "";
    }
// include coiniclestun prefix to communicate result is valid
#if defined(ANDROID) || defined(RPI)
    char buff[IFNAMSIZ + 1] = {0};
    snprintf(buff, sizeof(buff), "coiniclestun%u", num);
    return buff;
#else
    return "coiniclestun" + std::to_string(num);
#endif
  }
