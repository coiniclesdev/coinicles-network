...
namespace llarp
{
  namespace handlers
  {
      ...
      if(k == "mapaddr")
      {
        ...
        if(pos == std::string::npos)
        {
          llarp::LogError("Cannot map address ", v,
                          ...
                          "address.coinicles:ip.address.goes.here");
          return false;
        }
    ...
    static bool
    is_localhost_coinicles(const dns::Message &msg)
    {
      return msg.questions[0].IsName("localhost.coinicles");
    }
        ...
        dns::Message msg, std::function< void(dns::Message) > reply)
    {
      ...
      auto ReplyToCoiniclesDNSWhenReady = [self = this, reply = reply](
      ...
      if(msg.answers.size() > 0)
      {
        ...
        else if(answer.HasCNameForTLD(".coinicles"))
        {
          dns::Name_t qname;
          llarp_buffer_t buf(answer.rData);
          if(not dns::DecodeName(&buf, qname, true))
          ...
          auto replyMsg =
              std::make_shared< dns::Message >(clear_dns_message(msg));
          return ReplyToCoiniclesDNSWhenReady(addr, replyMsg, false);
        }
      }
      ...
      if(msg.questions[0].qtype == dns::qTypeMX)
      {
        // mx record
        service::Address addr;
        if(addr.FromString(qname, ".coinicles") || addr.FromString(qname, ".snode")
           || is_random_snode(msg) || is_localhost_coinicles(msg))
      ...
      else if(msg.questions[0].qtype == dns::qTypeCNAME)
      {
        ...
        else if(is_localhost_coinicles(msg))
        {
          size_t counter = 0;
              ...
              || msg.questions[0].qtype == dns::qTypeAAAA)
      {
        ...
        {  // empty reply but not a NXDOMAIN so that client can retry IPv4
          msg.AddNSReply("localhost.coinicles.");
        }
        ...
        else if(addr.FromString(qname, ".coinicles"))
        {
          if(isV4 && SupportsV6())
          {
          ...
          {
            return ReplyToCoiniclesDNSWhenReady(
      ...
      else if(msg.questions[0].qtype == dns::qTypePTR)
      {
        ...
        service::Address coiniclesAddr;
        if(FindAddrForIP(coiniclesAddr, ip))
        {
          msg.AddAReply(coiniclesAddr.ToString());
          reply(msg);
          return true;
        }
        msg.AddNXReply();
    ...
    TunEndpoint::ShouldHookDNSMessage(const dns::Message &msg) const
    {
      ...
      if(msg.questions.size() == 1)
      {
        /// hook every .coinicles
        if(msg.questions[0].HasTLD(".coinicles"))
          return true;
        /// hook every .snode
      ...
      {
        if(answer.HasCNameForTLD(".coinicles"))
          return true;
        if(answer.HasCNameForTLD(".snode"))
          return true;
      }
      ...
      if(tunif == nullptr)
      {
        ...
        auto ticker = [self, sendpkt]() {
          ...
          if(impl)
          {
            ...
            {
              // queue it to be sent over coiniclesnet
