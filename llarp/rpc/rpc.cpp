...
namespace llarp
{
  namespace rpc
  {
    ...
    struct CoiniclesPingHandler final : public CallerHandler
    {
      ~CoiniclesPingHandler() override = default;
      CoiniclesPingHandler(::abyss::http::ConnImpl* impl, CallerImpl* parent)
      ...
      HandleJSONResult(const nlohmann::json& result) override
      {
        ...
        {
          LogError("invalid result from coiniclesd ping, not an object");
          return false;
        }
        const auto itr = result.find("status");
        if(itr == result.end())
        {
          LogError("invalid result from coiniclesd ping, no result");
          return false;
        }
        if(not itr->is_string())
        {
          LogError("invalid result from coiniclesd ping, status not an string");
          return false;
        }
        const auto status = itr->get< std::string >();
        if(status != "OK")
        {
          LogError("coiniclesd ping failed: '", status, "'");
          return false;
        }
        LogInfo("coiniclesd ping: '", status, "'");
        return true;
      }
      void
      HandleError() override
      {
        LogError("Failed to ping coiniclesd");
      }
    };
    ...
    struct CallerImpl : public ::abyss::http::JSONRPC
    {
      ...
      AsyncCoiniclesPing()
      {
        LogInfo("Pinging Coiniclesd");
        nlohmann::json version(llarp::VERSION);
        nlohmann::json params({{"version", version}});
        QueueRPC("coiniclesnet_ping", std::move(params),
                 util::memFn(&CallerImpl::NewCoiniclesnetPingConn, this));
      }
      ...
      NewCoiniclesnetPingConn(abyss::http::ConnImpl* impl)
      {
        return new CoiniclesPingHandler(impl, this);
      }
    ...
    GetServiceNodeListHandler::HandleJSONResult(const nlohmann::json& result)
    {
      ...
      // If coiniclesd says tells us the block didn't change then nothing to do
    ...
    {
      hdr.emplace("User-Agent", "coiniclesnet rpc (YOLO)");
    }
