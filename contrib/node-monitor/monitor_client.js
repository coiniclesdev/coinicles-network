const coiniclesnet = require('./coiniclesnet') // currently using the 0.5 api
const fs = require('fs')
...
// while we monitor snode and coinicles addresses
...
var coiniclesnet_config = {
  binary_path: '../../coiniclesnet',
...
var coiniclesnet_version = 'unknown'
...
function addSnode(snode) {
  ...
  if (idx == -1) {
    ...
    function lookupSnode(snode) {
      coiniclesnet.lookup(snode, function (records) {
...
coiniclesnet.onMessage = function (data) {
  console.log(`monitor: ${data}`)
  var lines = data.split(/\n/)
  for (var i in lines) {
    var tline = lines[i].trim()
    // 	coiniclesnet-0.4.0-59e6a4bc (dev build)
    if (tline.match('coiniclesnet-0.')) {
      var parts = tline.split('coiniclesnet-0.')
      coiniclesnet_version = parts[1]
    ...
    if (tline.match('Set Up networking for')) {
      ...
      // default:9j4uido1ai7ucirbncbqii1395e8ccd6cjomo9cccp3ztx1ukwio.coinicles
    ...
    if (tline.match(' is built, took ')) {
      // path TX=8826efd28bede66c59782af9894eed08 RX=a994770a8c6d61700b8ca65e4a3adea3 on OBContext:default:3y3ch3m6c8xgmutxwe8fger6n963hn3mkn6tk14j67w1zdxj1n1y.coinicles-icxqqcpd3sfkjbqifn53h7rmusqa1fyxwqyfrrcgkd37xcikwa7y.coinicles is built, took 2321 ms
    ...
    //TX=53e833fcbad1395647f198201d4931e5 RX=b4768bc4b91b86b51b513319f443f538 on default:dnhi78pwrn6cd5yz83i83816nqudwym57s5x4bp3j4g5kbeigw5y.coinicles built latency=291
    ...
    //service/endpoint.cpp:652	default:kzow69uftho8ukm8g4kf88y5zka84azfrmfx5okkmrg7ucdz5d1o.coinicles IntroSet publish confirmed
    if (tline.match('IntroSet publish confirmed')) {
      score_total++
    }
  }
}
coiniclesnet.onError = function (data) {
...
function checkIP(cb) {
  coiniclesnet.getCoiniclesNetIP(function (ip) {
    if (ip === undefined) {
      checkIP(cb)
      return
    }
    //console.log('coiniclesnet interface ip', ip)
    cb(ip)
  })
}

coiniclesnet.startClient(coiniclesnet_config, function () {
  // coiniclesnet isn't necessarily running at this point
  console.log('Starting monitor')
  score_time = Date.now()
  checkIP(function (ip) {
    console.log('coiniclesnet interface ip', ip)
    coiniclesnet.findCoiniclesNetDNS(function (servers) {
    ...
    setTimeout(function () {
      console.log(coiniclesnet_config.binary_path, 'version', coiniclesnet_version, 'final score', score_total)
      process.exit()
