#!/usr/bin/env python3
#
# python wsgi application for managing many coiniclesnet instances
#

__doc__ = """coiniclesnet bootserv wsgi app
also handles webhooks for CI
run me with via gunicorn pycoiniclesnet.bootserv:app
"""
...
def handle_serve_coiniclesnet(modified_since, respond):
    l = BinHolder('coiniclesnet.zip')
    return l.serve(modified_since, respond)


def fetch_coiniclesnet(j, ref="staging", name="build:linux"):
    holder = BinHolder("coiniclesnet.zip")
    if 'builds' not in j:
        return False
    selected = None
    attrs = dict()
    ...
    if selected and 'id' in selected:
        url = 'https://gitlab.com/coiniclesproject/coinicles-network/-/jobs/{}/artifacts/download'.format(selected['id'])
        r = requests.get(url)
...
def app(environ, start_response):
    ...
    if method.upper() == "PUT" and request_body_size > 0:
        ...
        elif environ.get("PATH_INFO") == "/coiniclesnet.zip":
            return handle_serve_coiniclesnet(environ.get("HTTP_IF_MODIFIED_SINCE"),start_response)
        elif environ.get("PATH_INFO") == "/":
            return response("200 OK", "coiniclesnet bootserv", start_response)
        else:
