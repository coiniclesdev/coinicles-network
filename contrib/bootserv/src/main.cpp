...
main(int argc, char* argv[])
{
  ...
  const char* confFile = coiniclesnet::bootserv::Config::DefaultPath;
  coiniclesnet::bootserv::Config config;

  coiniclesnet::bootserv::Handler_ptr handler;
  ...
  if(RunCron)
    handler = coiniclesnet::bootserv::NewCronHandler(std::cout);
  else
    handler = coiniclesnet::bootserv::NewCGIHandler(std::cout);
