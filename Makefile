...
SHADOW_PLUGIN=$(BUILD_ROOT)/libshadow-plugin-coiniclesnet-shared.so
...
SHADOW_PLOT ?= $(PYTHON) $(SHADOW_SRC)/src/tools/plot-shadow.py -d $(REPO) CoiniclesNET -c $(SHADOW_CONFIG) -r 10000 -e '.*'
SHADOW_OPTS ?=
...
# cross compile?
CROSS ?= OFF
# build libcoiniclesnet-shared.so
SHARED_LIB ?= OFF
...
COVERAGE_OUTDIR ?= "$(TMPDIR)/coiniclesnet-coverage"
...
TARGETS = $(REPO)/coiniclesnet
SIGS = $(TARGETS:=.sig)
EXE = $(BUILD_ROOT)/daemon/coiniclesnet
...
shadow-run: shadow-build
	...
	cp $(SHADOW_PLUGIN) $(REPO)/libshadow-plugin-coiniclesnet.so
...
testnet: $(TESTNET_VENV)
	...
	$(PYTHON3) $(REPO)/contrib/testnet/genconf.py --bin=$(TESTNET_EXE) --svc=$(TESTNET_SERVERS) --clients=$(TESTNET_CLIENTS) --dir=$(TESTNET_ROOT) --out $(TESTNET_CONF) --ifname=$(TESTNET_IFNAME) --baseport=$(TESTNET_BASEPORT) --ip=$(TESTNET_IP) --netid=$(TESTNET_NETID) --coiniclesd='$(TESTNET_VENV)/bin/python $(REPO)/contrib/testnet/coiniclesd.py'
...
static: static-configure
	$(MAKE) -C '$(BUILD_ROOT)'
	cp $(EXE) $(REPO)/coiniclesnet-static
...
android-gradle-prepare: $(LIBUV_PREFIX)
	...
	echo "coiniclesnetCMake=$(REPO)/CMakeLists.txt" >> $(ANDROID_PROPS)
...
windows-debug: windows-debug-configure
	$(MAKE) -C '$(BUILD_ROOT)'
	cp '$(BUILD_ROOT)/daemon/coiniclesnet.exe' '$(REPO)/coiniclesnet.exe'
...
ifeq ($(CLANG),OFF)
	gcovr -r . --branches --html --html-details -o "$(COVERAGE_OUTDIR)/coiniclesnet.html"
else
...
docker-kubernetes:
	docker build -f docker/coinicles-svc-kubernetes.Dockerfile .

install-pycoiniclesnet:
	cd $(REPO)/contrib/py/pycoiniclesnet && $(PYTHON3) setup.py install

kubernetes-install: install install-pycoiniclesnet
