...
extern "C"
{
  ...
  /// packet writer to send packets to coiniclesnet internals
  struct llarp_vpn_writer_pipe;
  /// packet reader to recv packets from coiniclesnet internals
  ...
  struct llarp_vpn_io
  {
    ...
    /// user data
    void *user;
    /// hook set by user called by coiniclesnet core when coiniclesnet is done with the
    /// vpn io
    void (*closed)(struct llarp_vpn_io *);
    /// hook set by user called from coiniclesnet core after attempting to inject
  ...
  /// info about the network interface that we give to coiniclesnet core
  ...
  /// get the packet pipe for writing IP packets to coiniclesnet internals
  ...
  /// get the packet pipe for reading IP packets from coiniclesnet internals
  /// returns nullptr if llarp_vpn_io is nullptr or not initialized
  struct llarp_vpn_pkt_reader *
  llarp_vpn_io_packet_reader(struct llarp_vpn_io *io);

  /// blocking read on packet reader from coiniclesnet internals
  ...
  /// blocking write on packet writer to coiniclesnet internals
